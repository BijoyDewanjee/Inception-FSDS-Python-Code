...
print("""
     Welcome to Bijoy's Resturant. Please Order Food:
            Coffee: $2
            Pasta: $3
            Pizza: $5
            Burger: $6
...
if item1 in menu:
    ...
    print(f"You order {item1}. Your total order is ${total_price}")
else:
    print("Invalid item,Please order Something from the Menu")
another_order=input("Do you want tp add another item? (Yes/No) ").lower()
if another_order == "yes":
    item2 = input("Enter the name of the 2nd item you want to order: ")
    if item2 in menu:
        total_price += menu[item2]
        print(f"You order {item2}. Your total order is ${total_price}")
    else:
        print("Invalid item,Please order Something from the Menu")
